...
test('decode: empty magnet URIs return empty object', () => {
  ...
  expect(magnet(empty1)).toEqual(empty)
  expect(magnet(empty2)).toEqual(empty)
  expect(magnet(empty3)).toEqual(empty)
})

test('empty string as keys is okay', () => {
  const uri = 'magnet:?a=&b=&c='
  expect(magnet(uri)).toEqual(Object.assign({ a: '', b: '', c: '' }, empty))
})
...
test('decode: invalid magnet URIs return empty object', () => {
  ...
  expect(magnet(invalid1)).toEqual(empty)
  expect(magnet(invalid2)).toEqual(empty)
  expect(magnet(invalid3)).toEqual(empty)
  expect(magnet(invalid4)).toEqual(empty)
})
...
test('decode: invalid magnet URIs return only valid keys (ignoring invalid ones)', () => {
  ...
  expect(magnet(invalid1)).toEqual(Object.assign({ a: 'a' }, empty))
  expect(magnet(invalid2)).toEqual(Object.assign({ b: 'b' }, empty))
  expect(magnet(invalid3)).toEqual(Object.assign({ c: 'c' }, empty))
})
...
test('decode: peer-address remove duplicates', () => {
  ...
  expect(result['x.pe']).toEqual(['123.213.32.10:47450', '[2001:db8::2]:55013', '123.213.32.10:47450'])
