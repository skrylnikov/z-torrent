...
test('create nested torrent with array of buffers', async () => {
  ...
  expect(path.normalize(parsedTorrent.files[0].path)).toBe(path.normalize('multi/dir1/buf1.txt'))
  expect(parsedTorrent.files[0].length).toBe(2)
  expect(path.normalize(parsedTorrent.files[1].path)).toBe(path.normalize('multi/dir2/buf2.txt'))
