...
class utPex extends EventEmitter {
  ...
  onMessage(buf: Uint8Array): void {
    ...
    try {
      ...
      if (message.added) {
        compact2stringMulti(Buffer.from(message.added as Uint8Array)).forEach((peer, idx) => {
      ...
      if (message.added6) {
        compact2stringMulti6(Buffer.from(message.added6 as Uint8Array)).forEach((peer, idx) => {
          delete this._remoteDroppedPeers[peer]
          if (!(peer in this._remoteAddedPeers)) {
      ...
      if (message.dropped) {
        compact2stringMulti(Buffer.from(message.dropped as Uint8Array)).forEach((peer) => {
      ...
      if (message.dropped6) {
        compact2stringMulti6(Buffer.from(message.dropped6 as Uint8Array)).forEach((peer) => {
          delete this._remoteAddedPeers[peer]
          if (!(peer in this._remoteDroppedPeers)) {
