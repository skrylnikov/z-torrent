...
test('wire.use(ut_pex())', (t) => {
  ...
  t.ok((wire.ut_pex as unknown as { addPeer: unknown }).addPeer)
  t.ok((wire.ut_pex as unknown as { dropPeer: unknown }).dropPeer)
  t.ok(wire.ut_pex!.on)
  t.notOk((wire.ut_pex as unknown as { peers: unknown }).peers)
  t.end()
})
...
test('should ignore when addPeer receives an invalid peer', (t) => {
  ...
  t.notOk((pex as unknown as { _localAddedPeers: Record<string, unknown> })._localAddedPeers[peer])
  t.end()
})
...
test('should add to localAddedPeers when addPeer', (t) => {
  ...
  t.ok((pex as unknown as { _localAddedPeers: Record<string, unknown> })._localAddedPeers[peer])
  t.equal(
...
test('should add to localDroppedPeers when dropPeer', (t) => {
  ...
  t.ok((pex as unknown as { _localDroppedPeers: Record<string, unknown> })._localDroppedPeers[peer])
  t.end()
})
...
test('should ignore when onMessage added and address already in remoteAddedPeers', (t) => {
  const Extension = utPex()
  const wire = new Protocol()
  const pex = new Extension(wire as unknown as Parameters<typeof Extension>[0])
...
test('should add to remoteAddedPeers when onMessage added', (t) => {
  ...
  t.ok((pex as unknown as { _remoteAddedPeers: Record<string, unknown> })._remoteAddedPeers[peer])
  t.equal(
