...
if (args.includes('--stdin') || args.includes('-')) {
  stdin.buffer().then(onTorrentId).catch(error)
} else if (args.includes('--version') || args.includes('-v')) {
  console.log(require('../../package.json').version)
} else {
  onTorrentId(args[args.length - 1])
}

function onTorrentId(torrentId: string | Buffer) {
  remote(torrentId, function (err: Error | null, parsedTorrent?: any) {
...
function recursiveStringify(obj: any) {
  for (const key of Object.keys(obj)) {
    if (!Buffer.isBuffer(obj[key]) && typeof obj[key] === 'object' && obj[key] !== null) {
