...
test('adding a node updates the lastChange property', (t) => {
  ...
  t.notOk((dht._rpc.nodes as any).metadata.lastChange, 'lastChanged not set')

  setTimeout(() => {
    dht.addNode({ host: '127.0.0.1', port: 9999, id: common.randomId() })
    t.equal(typeof (dht._rpc.nodes as any).metadata.lastChange, 'number')
    t.ok((dht._rpc.nodes as any).metadata.lastChange > now, 'lastChange timestamp is older')
    dht.destroy()
  }, 50)
})
...
test('same node doesn´t change the lastChange property', (t) => {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      dht.addNode({ host: '127.0.0.1', port: 9999, id: nodeId })
      t.equal((dht._rpc.nodes as any).metadata.lastChange, lastChanged)
      dht.destroy()
    }, 1)
  }, 1)
})
