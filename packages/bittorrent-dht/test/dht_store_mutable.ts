...
test('local mutable put/get', (t) => {
  ...
  function ready() {
    ...
    dht.put(opts, (_, hash) => {
      t.equal(hash.toString('hex'), expectedHash.toString('hex'), 'hash of the public key')
...
test('salted multikey multiparty mutable put/get sequence', (t) => {
  ...
  function ready() {
    ...
    dht1.put(fopts, (err, hash) => {
      ...
      dht2.get(hash, (err, res) => {
        t.ifError(err)
        t.equal(res.v.toString('utf8'), fopts.v.toString('utf8'), 'got back what we put in')
        putSecondKey()
      })
    })
    ...
    function yetStillMore() {
      ...
      dht1.put(fopts, (err, hash) => {
        ...
        dht2.get(hash, (err, res) => {
          t.ifError(err)
          t.equal(res.v.toString('utf8'), fopts.v.toString('utf8'), 'update with a different salt')
        })
      })
    }
  }
  ...
  function ready() {
    ...
    dht1.put(opts, (err, hash) => {
      ...
      dht3.get(expectedHash, (err, res) => {
        t.ifError(err)
        t.equal(res.v.toString('utf8'), opts.v.toString('utf8'), 'got node 1 update from node 3')
      })
    })
  }
})
  ...
  function send(srci, dsti, value) {
    ...
    src.put(opts, (err, hash) => {
      ...
      dst.get(xhash, (err, res) => {
        t.ifError(err)
        t.equal(res.v.toString('utf8'), opts.v.toString('utf8'), `from ${srci} to ${dsti}`)
      })
    })
  }
})
