...
test('dht store test vectors - test 1 (mutable)', (t) => {
  ...
  const dht = new DHT({ bootstrap: false, verify: ed.verify } as any)
  t.teardown(() => {
    dht.destroy()
  })
  ...
  dht.listen(() => {
    dht.addNode({ host: '127.0.0.1', port: (dht.address() as any).port })
    dht.once('node', ready)
  })
  ...
  function ready() {
    ...
    dht.put(opts, (_, hash) => {
      ...
      dht.get(hash!, (err, res) => {
        ...
        t.equal(
          res!.v.toString('utf8'),
          (opts.v as string).toString('utf8'),
...
test('dht store test vectors - test 2 (mutable with salt)', (t) => {
  ...
  function ready() {
    ...
    dht.put(opts, (_, hash) => {
      t.equal(hash!.toString('hex'), '411eba73b6f087ca51a3795d9c8c938d365e32c1', 'hashes match')
