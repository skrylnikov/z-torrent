...
test('create single file torrent', (t) => {
  ...
  createTorrent(fixtures.leaves.contentPath, async (err, torrent) => {
    ...
    t.equals(
      path.normalize(parsedTorrent.files[0].path),
...
test('create single file torrent from buffer', (t) => {
  ...
  createTorrent(Buffer.from('blah'), { name: 'blah.txt' }, async (err, torrent) => {
    ...
    try {
      ...
      t.fail(`failed to parse created torrent: ${(err as Error).message}`)
    }
  })
})
...
test('create multi file torrent', (t) => {
  ...
  createTorrent(
    ...
    async (err, torrent) => {
      ...
      t.deepEquals(parsedTorrent.pieces, ['1f74648e50a6a6708ec54ab327a163d5536b7ced'])
      hash(parsedTorrent.infoBuffer, 'hex').then((hash) => {
...
test('create multi file torrent with nested directories', (t) => {
  ...
  const startTime = Date.now()
  createTorrent(
    fixtures.lotsOfNumbers.contentPath,
    {
      // force piece length to 32KB so info-hash will
      // match what transmission generated, since we use
      // a different algo for picking piece length
    ...
    async (err, torrent) => {
      ...
      t.deepEquals(
        path.normalize(parsedTorrent.files[0].path),
        path.normalize('lots-of-numbers/big numbers/10.txt')
      )
      t.deepEquals(parsedTorrent.files[0].length, 2)

      t.deepEquals(
        path.normalize(parsedTorrent.files[1].path),
        path.normalize('lots-of-numbers/big numbers/11.txt')
      )
      t.deepEquals(parsedTorrent.files[1].length, 2)

      t.deepEquals(
        path.normalize(parsedTorrent.files[2].path),
        path.normalize('lots-of-numbers/big numbers/12.txt')
      )
      t.deepEquals(parsedTorrent.files[2].length, 2)

      t.deepEquals(
        path.normalize(parsedTorrent.files[3].path),
        path.normalize('lots-of-numbers/small numbers/1.txt')
      )
      t.deepEquals(parsedTorrent.files[3].length, 1)

      t.deepEquals(
        path.normalize(parsedTorrent.files[4].path),
        path.normalize('lots-of-numbers/small numbers/2.txt')
      )
      t.deepEquals(parsedTorrent.files[4].length, 2)

      t.deepEquals(
        path.normalize(parsedTorrent.files[5].path),
        path.normalize('lots-of-numbers/small numbers/3.txt')
      )
      ...
      t.deepEquals(parsedTorrent.pieces, ['47972f2befaee58b6f3860cd39bd56cb33a488f0'])
      hash(parsedTorrent.infoBuffer, 'hex').then((hash) => {
...
test('create multi file torrent with array of paths', (t) => {
  ...
  createTorrent(
    ...
    async (err, torrent) => {
      ...
      t.deepEquals(parsedTorrent.pieces, ['1c4e1ba6da4d771ff82025d7cf76e8c368c6c3dd'])
      hash(parsedTorrent.infoBuffer, 'hex').then((hash) => {
