...
test('Limit download speed by methods when tcp connection', async () => {
...
test('Limit upload speed by methods when tcp connection', async () => {
  await testSpeed({}, { uploadLimit: UPLOAD_SPEED_LIMIT }, (_, uploadSpeeds) => {
    expect(uploadSpeeds.every((s) => s <= UPLOAD_SPEED_LIMIT)).toBeTruthy()
  })
})

test('Limit download speed by methods when utp connection', async () => {
  await testSpeed(
...
test('Limit upload speed by methods when utp connection', async () => {
  await testSpeed(
    { utp: true },
