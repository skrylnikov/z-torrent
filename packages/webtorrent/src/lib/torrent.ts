...
export default class Torrent extends EventEmitter {
  ...
  _registerPeer(newPeer: Peer): void {
    newPeer.on('download', (downloaded: number) => {
      ...
      ;(this.client as any)._downloadSpeed(downloaded)
      this.emit('download', downloaded)
      if (this.destroyed) return
      ;(this.client as any).emit('download', downloaded)
    })
    ...
    newPeer.on('upload', (uploaded: number) => {
      ...
      ;(this.client as any)._uploadSpeed(uploaded)
      this.emit('upload', uploaded)
      if (this.destroyed) return
      ;(this.client as any).emit('upload', uploaded)
    })
  ...
  _addPeer(addr: string, type: string, source: string): Peer | null {
    ...
    const newPeer =
      type === 'utp'
        ? Peer.createUTPOutgoingPeer(addr, this, (this.client as any).throttleGroups, source as any)
        : Peer.createTCPOutgoingPeer(addr, this, (this.client as any).throttleGroups, source as any)
  ...
  _addWebPeer(peer: any, source: string): void {
    ...
    const newPeer = Peer.createWebRTCPeer(peer, this, (this.client as any).throttleGroups, source as any)
  ...
  _onWire(wire: any, addr?: string): void {
    ...
    if ((this.client as any).dht && (this.client as any).dht.listening) {
      wire.on('port', (port: number) => {
        if (this.destroyed || (this.client as any).dht.destroyed) return
        if (!(wire as any).remoteAddress) return this._debug('ignoring PORT from peer with no address')
        if (port === 0 || port > 65536) return this._debug('ignoring invalid PORT from peer')
        this._debug('port: %s (from %s)', port, addr)
        ;(this.client as any).dht.addNode({ host: (wire as any).remoteAddress, port })
      })
    }
    ...
    if ((this.client as any).utPex && !(this as any).private) {
      wire.use(utPex())
  ...
  _onWireWithMetadata(wire: any): void {
    ...
    wire.on('request', (index: number, offset: number, length: number, cb: (err?: Error, chunk?: Uint8Array) => void) => {
    ...
    if ((wire as any).peerExtensions?.dht && (this.client as any).dht?.listening) {
      wire.port((this.client as any).dht.address().port)
    }
  ...
  _update(): void {
    if (IDLE_CALLBACK) {
      ;(IDLE_CALLBACK as any)(() => this._updateWireWrapper(), { timeout: 250 })
    } else {
  ...
  _updateWire(wire: any): boolean {
    ...
    const getBlockPipelineLength = (w: any, duration: number) =>
      Math.max(2, 2 + Math.ceil(duration * (typeof w.downloadSpeed === 'function' ? w.downloadSpeed() : 0) / Piece.BLOCK_LENGTH))
    const getPiecePipelineLength = (w: any, duration: number, pl: number) =>
      Math.max(1, 1 + Math.ceil(duration * (typeof w.downloadSpeed === 'function' ? w.downloadSpeed() : 0) / pl))
    const minOutstanding = isWebSeed
      ? Math.min(getPiecePipelineLength(wire, PIPELINE_MIN_DURATION, this.pieceLength), this.maxWebConns)
      : getBlockPipelineLength(wire, PIPELINE_MIN_DURATION)
    const maxOutstanding = isWebSeed
      ? Math.min(getPiecePipelineLength(wire, PIPELINE_MAX_DURATION, this.pieceLength), this.maxWebConns)
    ...
    for (let i = 0; i < selections.length; i++) {
      ...
      const end = next.to
      for (let piece = end; piece >= start; --piece) {
  ...
  _request(wire: any, index: number, hotswap: boolean): boolean {
    ...
    wire.request(index, chunkOffset, chunkLength, async (err: Error | null, chunk?: Uint8Array) => {
      if (this.destroyed) return
      if (!this.ready) return this.once('ready', () => wire.request(index, chunkOffset, chunkLength, arguments[3]))
      ...
      try {
        ...
        if (this.destroyed) return
        const expectedHex = (this._hashes as any)[index] as string
        if (hex === expectedHex) {
  ...
  _hotswap(wire: any, index: number): boolean {
    ...
    for (let i = 0; i < r.length; i++) {
      ...
      const otherSpeed = typeof otherWire.downloadSpeed === 'function' ? otherWire.downloadSpeed() : 0
    ...
    for (let i = 0; i < minWire.requests?.length; i++) {
      ...
      ;(this.pieces as any[])[index].cancel((req.offset / Piece.BLOCK_LENGTH) | 0)
    }
  ...
  _hasMorePieces(threshold: number): boolean {
    let count = 0
    for (let index = 0; index < this.pieces.length; index++) {
      if (this.bitfield!.get(index)) {
        count += 1
  ...
  _onMetadata(parsedTorrent: ParsedTorrent | this): void {
    ...
    if ((this.client as any).enableWebSeeds && this.urlList) {
    ...
    this.bitfield = new BitField(
      hasStartupBitfield ? new Uint8Array(this._startupBitfield as ArrayLike<number>) : this.pieces.length
    )
  ...
  _verifyPiece(index: number, cb: (err?: Error, isVerified?: boolean) => void): void {
    ...
    const getOpts: any = index === (this.pieces as any[]).length - 1 ? { length: this.lastPieceLength } : {}
    this.store.get(index, getOpts, async (err: Error | null, buf: Buffer) => {
      if (this.destroyed) return cb(new Error('torrent is destroyed'))
  ...
  _markUnverified(index: number): void {
    const len = index === (this.pieces as any[]).length - 1 ? this.lastPieceLength : this.pieceLength
  ...
  _rechoke(): void {
    ...
    const wireStack = this.wires
      .map((wire) => ({
        wire,
        random: Math.random(),
        downspeed: typeof (wire as any).downloadSpeed === 'function' ? (wire as any).downloadSpeed() : 0,
      }))
      .sort((objA, objB) => {
        if (objA.downspeed !== objB.downspeed) return objA.downspeed - objB.downspeed
        const aup = typeof (objA.wire as any).uploadSpeed === 'function' ? (objA.wire as any).uploadSpeed() : 0
        const bup = typeof (objB.wire as any).uploadSpeed === 'function' ? (objB.wire as any).uploadSpeed() : 0
    ...
    wireStack
      .filter((wire) => wire !== this._rechokeOptimisticWire)
  ...
  _destroy(err?: Error, opts?: any, cb?: () => void): void {
    ...
    ;(this.client as any)._remove(this)
    ...
    parallel(tasks, (parallelErr) => {
      if (err) {
        if (this.listenerCount('error') === 0) {
          ;(this.client as any).emit('error', err)
        } else {
          this.emit('error', err)
        }
      }
  ...
  addWebSeed(urlOrConn: string | any): void {
    ...
    const newPeer = Peer.createWebSeedPeer(conn, id, this, (this.client as any).throttleGroups)
