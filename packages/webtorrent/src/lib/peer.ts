...
export default class Peer extends EventEmitter {
  ...
  sendPe3(): void {
    this.wire!.sendPe3((this.swarm as any).infoHash)
    this.sentPe3 = true
  }

  onPe3(infoHashHash: string): void {
    if (this.swarm) {
      if ((this.swarm as any).infoHashHash !== infoHashHash) {
  ...
  sendPe4(): void {
    this.wire!.sendPe4((this.swarm as any).infoHash)
    this.sentPe4 = true
  }
  ...
  onHandshake(infoHash: string, peerId: string): void {
    ...
    if (peerId === (this.swarm as any).client.peerId) {
    ...
    ;(this.swarm as any)._onWire(this.wire, addr)
  ...
  handshake(): void {
    const opts = {
      dht: (this.swarm as any).private ? false : !!(this.swarm as any).client.dht,
      fast: true,
    }
    this.wire!.handshake(this.swarm!.infoHash, (this.swarm as any).client.peerId, opts)
  ...
  destroy(err?: Error): void {
    ...
    if (swarm && wire) {
      arrayRemove((swarm as any).wires, (swarm as any).wires.indexOf(wire))
    }
    if (conn) {
      conn.on('error', () => {})
      conn.destroy()
    ...
    if (swarm) (swarm as any).removePeer(this.id)
  }
