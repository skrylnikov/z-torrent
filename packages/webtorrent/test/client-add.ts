...
test('client.add: magnet uri, utf-8 string', (t) => {
  ...
  torrent.on('infoHash', async () => {
    ...
    await client.remove(fixtures.leaves.magnetURI, (err) => {
...
test('client.add: torrent file, buffer', (t) => {
  ...
  torrent.on('infoHash', async () => {
    ...
    await client.remove(fixtures.leaves.torrent, (err) => {
      t.error(err, 'torrent destroyed')
    })
    t.equal(client.torrents.length, 0)
...
test('client.add: info hash, hex string', (t) => {
  ...
  torrent.on('infoHash', async () => {
    ...
    await client.remove(fixtures.leaves.parsedTorrent.infoHash, (err) => {
      t.error(err, 'torrent destroyed')
    })
    t.equal(client.torrents.length, 0)
...
test('client.add: info hash, buffer', (t) => {
  ...
  torrent.on('infoHash', async () => {
    t.equal(torrent.infoHash, fixtures.leaves.parsedTorrent.infoHash)
    t.ok(
      torrent.magnetURI.indexOf(`magnet:?xt=urn:btih:${fixtures.leaves.parsedTorrent.infoHash}`) ===
        0
    )

    await client.remove(Buffer.from(fixtures.leaves.parsedTorrent.infoHash, 'hex'), (err) => {
      t.error(err, 'torrent destroyed')
    })
    t.equal(client.torrents.length, 0)
...
test('client.add: parsed torrent, from `parse-torrent`', (t) => {
  ...
  torrent.on('infoHash', async () => {
    ...
    await client.remove(fixtures.leaves.parsedTorrent, (err) => {
      t.error(err, 'torrent destroyed')
    })
    t.equal(client.torrents.length, 0)
