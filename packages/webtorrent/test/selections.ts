        ...
        (s) => {
          ...
          for (const otherFn of Object.keys(testCases)) {
            if (otherFn !== functionName) {
              t.equal((testCases as any)[otherFn].fn(newItem, existing), false)
            }
          }
          s.end()
        }
...
function assertArrayContentsEqual(t: Test, actual: SelectionItem[], expected: SelectionItem[]) {
