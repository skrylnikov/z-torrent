/**
 * Piece selection logic — platform-agnostic.
 * Manages which pieces to download and their priority.
 */
...
export class Selections {
  ...
  /** For testing: returns items as {from, to}[] */
  toArray(): Array<{ from: number; to: number }> {
    return this._items.map(({ from, to }) => ({ from, to }))
  }
