...
export abstract class ServerBase {
  ...
  abstract createFileBody(file: File, req: Request, range: { start: number; end: number } | null): unknown
  ...
  serveFile(file: File, req: Request, res: Response): Response {
    ...
    if (Array.isArray(range)) {
      ...
      res.headers['Content-Range'] =
        `bytes ${rangeObj.start}-${rangeObj.end}/${file.length}`
  ...
  async onRequest(req: Request, cb: (res: Response) => void): Promise<void> {
    ...
    const handleRequest = async (): Promise<Response> => {
      if (pathname === '') {
        return ServerBase.serveIndexPage(res, this.client.torrents as TorrentWithFiles[], this.pathname)
      }
      ...
      const file = torrentWithFiles.files.find(
        (f) => f.path.replace(/\\/g, '/') === filePath
      )
      if (!file) {
        return ServerBase.serve404Page(res)
      }
