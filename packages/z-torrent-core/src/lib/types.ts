...
export interface TorrentWire {
  ...
  store: { get: (index: number, opts: unknown, cb: (err: Error | null, chunk?: Uint8Array) => void) => void } | null
  destroyed: boolean
  wires: Wire[]
  on(event: string, fn: (...args: unknown[]) => void): void
  removeListener(event: string, fn: (...args: unknown[]) => void): void
  emit(event: string, ...args: unknown[]): boolean
  _select(start: number, end: number, priority: number, notify: (() => void) | null, isStreamSelection: boolean): void
