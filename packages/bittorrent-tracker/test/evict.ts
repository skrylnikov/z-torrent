...
function serverTest(
  ...
  const opts: { serverType: 'http' | 'udp' | 'ws'; peersCacheLength?: number } = {
    serverType,
    peersCacheLength: 2,
  }
...
test('evict: ipv4 server', (t: Test) => {
  serverTest(t, 'http', 'inet')
})

test('evict: http ipv6 server', (t: Test) => {
  serverTest(t, 'http', 'inet6')
})

test('evict: udp server', (t: Test) => {
  serverTest(t, 'udp', 'inet')
})

test('evict: ws server', (t: Test) => {
