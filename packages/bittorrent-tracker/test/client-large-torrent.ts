...
function testLargeTorrent(t: Test, serverType: 'http' | 'udp' | 'ws'): void {
  t.plan(9)
...
test('http: large torrent: client.start()', (t: Test) => {
  testLargeTorrent(t, 'http')
})

test('udp: large torrent: client.start()', (t: Test) => {
  testLargeTorrent(t, 'udp')
})

test('ws: large torrent: client.start()', (t: Test) => {
