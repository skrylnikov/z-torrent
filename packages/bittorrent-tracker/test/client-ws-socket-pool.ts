...
test('ensure client.destroy() callback is called with re-used websockets in socketPool', (t: Test) => {
  t.plan(4)
  ...
  common.createServer(t, 'ws', (server, announceUrl) => {
    ...
    client1.once('update', () => {
      ...
      client2.once('update', () => {
        ...
        client1.destroy(() => {
          t.pass('got client1 destroy callback')
          client2.destroy(() => {
            t.pass('got client2 destroy callback')
            server.close()
          })
        })
      })
